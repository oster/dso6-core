...
public class TextFileFunctions {
    ...
    public Command transp(UpdateTextFile c1, UpdateTextFile c2)
        throws Exception {
        Command res = null;
        Method m = null;
        ...
        if (s1 < s2) {
            ...
            if (s1 > s2) {
                ...
                if (c1.equals(c2)) {
                    ...
                    // o1 == 02
                    //Logger.getLogger("blurp").info("conflict
                    // addblock/addblock, c1:" + c1.toStringVerbose() + ", c2:"
                    // + c2.toStringVerbose());
                    //Logger.getLogger("blurp").info("ticket a1:" +
                    // c1.getTicket() + ",a2:" + c2.getTicket());
                    // generate DelBlock (to remove block added before)
                    DelBlock d = new DelBlock(c2, ws);

                    // generate AddBlock (add block merging addblocks from c1
                    // and c2);
                    ...
                    for (Iterator i = cBefore.getContent().iterator(); i.hasNext();) {
                        contents.add(CONFLICT_BLOC_PADDING + ((String) i.next()));
                    }

                    contents.add(CONFLICT_BLOC_SPLIT + cBefore.getWsName() + CONFLICT_BLOC_ORIGINE_SEPARATOR + cAfter.getWsName()); // +
                                                                                                                                    // "\n");

                    for (Iterator i = cAfter.getContent().iterator(); i.hasNext();) {
                        contents.add(CONFLICT_BLOC_PADDING + ((String) i.next()));
                    }
    ...
    public Command tf(AddBlock c1, DelBlock c2) throws Exception {
        ...
        if (s1 <= s2) { //PATCH
            ...
            if (s1 > e2) {
                ...
                for (Iterator i = c2.getOldContent().iterator(); i.hasNext();) {
                    contents.add(CONFLICT_BLOC_PADDING + ((String) i.next()));
                }
                ...
                for (Iterator i = c1.getContent().iterator(); i.hasNext();) {
                    contents.add(CONFLICT_BLOC_PADDING + ((String) i.next()));
                }
    ...
    public Command tf(DelBlock c1, AddBlock c2) throws Exception {
        ...
        if (s1 >= s2) {
            ...
            if (e1 < s2) {
                ...
                for (Iterator i = c1.getOldContent().iterator(); i.hasNext();) {
                    contents.add(CONFLICT_BLOC_PADDING + ((String) i.next()));
                }
                ...
                for (Iterator i = c2.getContent().iterator(); i.hasNext();) {
                    contents.add(CONFLICT_BLOC_PADDING + ((String) i.next()));
                }
