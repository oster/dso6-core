...
public class XMLProgressView extends JPanel implements MessageWriter {
    ...
    public XMLProgressView(XMLLogHandler xmlLogHandler) {
        context = xmlLogHandler.getContext();
        xmlLogHandler.addMessageListener(this);

        //
        localBar = context.getLocalProgressBar();
        globalBar = context.getGlobalProgressBar();
    ...
    public void printMessage(boolean global, String message) {
        if (global) {
            globalText.setText(message);
        } else {
            localText.setText(message);
        }
    }
