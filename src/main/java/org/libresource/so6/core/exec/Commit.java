...
public class Commit {
     ...
     * @throws Exception
     */
    public Commit(String wscPath, String comment) throws Exception {
        ...
        if (this.ws.getProperty("so6.clienti.name").equals("org.libresource.so6.core.client.ClientIServletImpl")) {
            CheckWscParameters checkWscParameters = new CheckWscParameters(wscPath);
            Properties newProps = checkWscParameters.editProperties();
    ...
    public static void main(String[] args) throws Exception {
        if (args.length < 1) {
            ...
            Commit commit = new Commit(wsPath, comment);
            commit.execute();
