...
public class FileSystemFunctions {
			...
			if ((c2 instanceof Remove) || (c2 instanceof Rename)) {
				try {
					...
					m = this.getClass().getMethod("tf",
							new Class[] { c1.getClass().getSuperclass().getSuperclass(), c2.getClass() });
				}
			} else {
				try {
					m = this.getClass().getMethod("tf", new Class[] { c1.getClass().getSuperclass(), c2.getClass() });
					...
					try {
						m = this.getClass().getMethod("tf",
								new Class[] { c1.getClass(), c2.getClass().getSuperclass() });
	...
	public Command tf(AddFile c1, UpdateBinaryFile c2) throws Exception {
		return c1;
	}
	 ...
	 * UpdateBinaryFile
	 */
	public Command tf(UpdateBinaryFile c1, AddFile c2) throws Exception {
		return c1;
	}
	...
	public Command tf(UpdateBinaryFile c1, UpdateBinaryFile c2) throws Exception {
		if (c1.getPath().equals(c2.getPath())) {
	...
	public String replacePath(String pathToRename, String newPathToApply) {
		...
		assert pathToRename.startsWith(commonPath) : "bad replacement of path '" + pathToRename + "' with '"
				+ newPathToApply + "'";
